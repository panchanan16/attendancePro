...
async function openStudentBox(mainid) {
   ...
   const data = await method.GET_POST(`apiv1/get-student-with-filter?dep=${param[0].toUpperCase()}&sem=${param[1]}`, 'GET', undefined, undefined)
   if (data.length > 0) {
     ...
     data.forEach(stud => {
        ...
        <td class="p-[1rem] border-collapse text-left">1st sem</td>
        <td class="p-[1rem] border-collapse text-left flex items-center gap-3">
            <span>2023</span> <input type="checkbox" data-stdid="${stud._id}" name="" id="student-row-edit"></td>
        </tr>`
